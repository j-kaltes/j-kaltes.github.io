...
int changehost(int index,JNIEnv *env,jobjectArray jnames,int nr,bool detect,string_view port,const bool sendnums,const bool sendstream,const bool sendscans,const bool restore,const bool receive,const bool activeonly,string_view pass,uint32_t starttime,bool passiveonly,const char *label=nullptr,const bool testip=true,bool startthreads=true) {
	LOGGER("changehost(%d,sendnums=%d,sendstream=%d,sendscans=%d,receive=%d,activeonly=%d,passiveonly=%d,label=%s\n",index,sendnums,sendstream,sendscans,receive,activeonly,passiveonly,label);
...
void backupthread(int index) {
	...
	{
	//namehost name(&getupdatedata()->allhosts[index].host);
	...
	uintptr_t current=0;
	   while(true) {
		  if(con_vars[h]->dobackup&wakeend)  {
			LOGGER("%d: end backupthread 1 %d %p\n",h,agettid(),con_vars[h]);
		  ...
		  if(current&wakeend)  {
			getupdatedata()->tosend[h].close();
			LOGGER("%d: end backupthread 2 %d %p\n",h,agettid(),con_vars[h]);
			delete con_vars[h];
			con_vars[h]=nullptr;
			LOGGER("after delete\n");
			return;
		...
		if(current&wakereconnect&&!passive)  {
			
			getupdatedata()->tosend[h].close();
			getupdatedata()->tosend[h].open();
			} 
		if(current&wakesend&&!passive) {
			sendwakesender(h);
			}
		if(current&wakestreamsend&&!passive) {
	...
	(didnums=updateproc(con_vars[h],current&wakenums,getupdatedata()->tosend[h],&updateone::updatenums))&&
		(didstream=	updateproc(con_vars[h],current&wakestream,getupdatedata()->tosend[h],&updateone::updatestream))&&
		(didscans=		updateproc(con_vars[h],current&wakescan,getupdatedata()->tosend[h],&updateone::updatescans))&&
		(didupdate=		updateproc(con_vars[h],current&wakeall,getupdatedata()->tosend[h],&updateone::update));
	//if(didnums&1||didstream&1||didscans&1||didupdate&1)
	uint16_t command=((didnums&1)?wakenums:0)|(didstream&1?wakestream:0)|(didscans&1?wakescan:0)|(didupdate&1?wakeall:0);
	if(command)
