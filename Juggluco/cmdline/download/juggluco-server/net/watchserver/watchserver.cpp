...
bool sendall(int sock ,const char *buf,int buflen) ;
...
static std::string_view getdeltaname(float rate) {
	if(rate>=3.5f)
                return "DoubleUp";
	if(rate>=2.0f)
                return "SingleUp";
...
char *Sgvinterpret::firstdata(char *outiter,time_t starttime,uint32_t dattime) const {
	...
	if(sensorinfo) {
		...
		struct tm tmtim;
		 ...
		 outiter+=sprintf(outiter,R"PRE(,"sensor_status":"%02d-%02d-%02d %02d:%02d (%dd %dh)")PRE",tmtim.tm_mday,tmtim.tm_mon+1,tmtim.tm_year-100,tmtim.tm_hour,tmtim.tm_min,days,hourleft);
		 }
	return outiter;
	}
...
char *Sgvinterpret::writeitem(char *outiter,int datit, const ScanData *iter,const char *sensorname,const time_t starttime) const {
	LOGGER("writeitem %d\n",datit);
	 ...
	 double delta= iter->ch*deltatimes;
...
//	return sendall(sock, startheader, totlen);
bool sendall(int sock ,const char *buf,int buflen) {
